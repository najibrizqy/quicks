    ...
    return (
        <div className="flex space-x-4 items-end mt-3">
            <div className={`transition-all duration-300 ${isExpanded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4'} flex flex-row items-center space-x-4`}>
                <div onClick={openTasks} className="flex flex-col items-center">
                <span className={`text-white text-sm mb-2 ${!isExpanded && 'hidden'}`}>Task</span>
                <button className="flex items-center justify-center bg-white p-4 w-[60px] h-[60px] rounded-full">
                    <img src={taskIcon} alt="Task" className="h-6 w-6" />
                </button>
                </div>
                <div className="flex flex-col items-center">
                <span className={`text-white text-sm mb-2 ${!isExpanded && 'hidden'}`}>Inbox</span>
            ...
            <button onClick={toggleExpand} className="flex items-center justify-center bg-blue text-white p-4 w-[68px] h-[68px] rounded-full">
